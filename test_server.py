# Copyright 2013 Google Inc. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#   http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
"""Runs a test server for End-To-End."""

__author__ = "koto@google.com (Krzysztof Kotowicz)"

import atexit
import BaseHTTPServer
import cgi
import fnmatch
import os
import os.path
import shutil
import SimpleHTTPServer
import StringIO
import tempfile

server_address = ("127.0.0.1", 8000)
DEPS_F, DEPS_PATH = tempfile.mkstemp("test_js_deps-runfiles.js", "e2e")


def generate_deps_file():
  """Generates temporary dependencies file for running tests.

  Dependencies file in End-To-End does not include dependencies for zlib.js.
  We fix that by creating a temporary dependencies file.
  """

  dest = os.fdopen(DEPS_F, "wb")
  with open("src/javascript/crypto/e2e/deps.js", "rb") as f:
    shutil.copyfileobj(f, dest)
  with open("lib/zlib.js/deps.js", "r") as f2:
    for line in f2:
      dest.write(line.replace("../", "../../zlib-root/"))


def exit_handler():
  """Removes the temporary file on exit."""
  try:
    os.unlink(DEPS_PATH)
  except IOError:
    pass


class TestServerRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
  """Request handler for End-To-End test server."""

  DIRECTORY_MAP = {
      "/src/javascript/closure/": "lib/closure-library/closure/goog/",
      "/src/third_party/closure/": "lib/closure-library/third_party/closure/",
      "/src/zlib-root/": "lib/zlib.js/"
  }

  def list_directory(self, path):
    """Lists only _test.html files."""
    test_files = []
    for root, _, files in os.walk("src/"):
      for f in fnmatch.filter(files, "*_test.html"):
        test_files.append(os.path.join(root, f))
    out = StringIO.StringIO()
    out.write("<h2>End-To-End test server</h2>")
    out.write("<ul>")
    for f in test_files:
      out.write("<li><a href=\"%s\">%s</a>\n" % (f, cgi.escape(f)))
    out.write("</ul>")
    out.seek(0)
    self.send_response(200)
    self.send_header("Content-type", "text/html")
    self.end_headers()
    return out

  def translate_path(self, path):
    """Serves files from different directories."""
    if path.endswith("test_js_deps-runfiles.js"):
      return DEPS_PATH
    for prefix, dest_dir in TestServerRequestHandler.DIRECTORY_MAP.items():
      if path.startswith(prefix):
        return dest_dir + path[len(prefix):]
    return SimpleHTTPServer.SimpleHTTPRequestHandler.translate_path(
        self, path)

generate_deps_file()
atexit.register(exit_handler)
httpd = BaseHTTPServer.HTTPServer(server_address, TestServerRequestHandler)
print "Starting test server at http://%s:%d" % server_address
httpd.serve_forever()
